<template>
  ...
  <div v-show="!loading">
      ...
      <router-link :to="{ name: 'productFeatures', params: { id: entry.eid } }" tag="button" class="btn btn-light float-right"  exact>Product Features</router-link>  

    <h3>Product: {{entry.title}}</h3>
    ...
    <div class="form-group">
        <label>SKU</label>
        <input type="text" class="form-control" v-model="entry.sku" @input="pendingChanges"/>
    </div>
